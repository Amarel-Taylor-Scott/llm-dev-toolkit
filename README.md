...
| [`gradnoise`](https://github.com/Amarel-Taylor-Scott/gradnoise) | The gradient noise scale (McCandlish et al.) + per-module SNR from microbatch gradients — how noisy your gradients are and the largest useful batch size, via unbiased two-batch estimators; checked against ground truth (recovers a known noise scale within ~1%, confirms the |g_B|²=|G|²+S/B scaling law) _(new)_ |
| [`mapelites`](https://github.com/Amarel-Taylor-Scott/mapelites) | MAP-Elites quality-diversity search — keep the best solution per behavior-space cell instead of one global optimum, illuminating a diverse archive; gets coverage 1.0 AND near-optimal quality where random search covers without quality and a hill-climber gets quality in one cell _(new)_ |
| [`stablerank`](https://github.com/Amarel-Taylor-Scott/stablerank) | Robust effective-dimensionality of a matrix — stable rank, effective/entropy rank, spectral entropy, participation ratio, numerical rank + an energy-capture curve; the hard rank is fooled by noise (rank-5-plus-noise reads as 50) while these see the true ~5. Exact on known ranks _(new)_ |
